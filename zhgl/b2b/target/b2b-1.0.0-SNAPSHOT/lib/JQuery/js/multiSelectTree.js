...
第三个参数 宽度
第四个参数 根接点ID
第五个参数 图片（说明 ： 没有第五个参数就使用默认值图片）
第六个参数 上下文根
*/
function dhtmlXTreeObject(htmlObject,width,height,rootId,icon,ctxPath){
 ...
 this.minusArray=new Array("minus2.gif","minus3.gif","minus4.gif","minus1.gif","minus5.gif");
 this.plusArray=new Array("plus2.gif","plus3.gif","plus4.gif","plus1.gif","plus5.gif");
 this.imageArray=new Array("leaf.gif","folderOpen.gif","folderClosed.gif");
 }else{
 this.checkArray= icon.checkArray;
 this.lineArray= icon.lineArray;
 this.minusArray= icon.minusArray;
 this.plusArray= icon.plusArray;
 this.imageArray = icon.imageArray;
 }
 ...
 this.enableCheckBoxes(1);
//允许三种状态的复选框（全选、不选、部分选中）
this.enableThreeStateCheckboxes(true);
this.setImagePath(((ctxPath == null)?"..":ctxPath)+"/img/");
 return this;
};
...
dhtmlXTreeObject.prototype._correctCheckStates=function(dhtmlObject){
 ...
 var flag1=0;var flag2=0,flag3=0;
 if(act.length<2)return;
 for(var i=1;i<act.length;i++){
	// alert(act[i].nodem.checkstate);
 if(act[i].nodem.checkstate==0)flag1=1;
 else if(act[i].nodem.checkstate==1)flag2=1;
 else if(act[i].nodem.checkstate==3) flag3=1;
 else{flag1=1;flag2=1;break;}

 }


 //alert(flag1+"s"+flag2+"b"+flag3);
   if(flag3)this._setCheck(dhtmlObject,3);
else if((flag1)&&(flag2))this._setCheck(dhtmlObject,"notsure");
 ...
 else if(this.parentObject.checkstate==3)this.treeNod._setSubChecked(3,this.parentObject);
 else this.treeNod._setSubChecked(true,this.parentObject);}
 else{
 if(this.parentObject.checkstate==1)this.treeNod._setCheck(this.parentObject,false);
 else if(this.parentObject.checkstate==3)this.treeNod._setCheck(this.parentObject,3);
...
/**设置图片路径
第一个参数为图片的路径 （可以不设置,不设置就使用默认的路径 ../img/）
*/
 ...
 for(var i=0;i<Count;i++)
{
 ...
 var z=sNode.htmlNode.childNodes[0].childNodes[0].childNodes[1].childNodes[0];
  ...
  if(state ==3)sNode.checkstate=3;
else if(state=="notsure")sNode.checkstate=2;
 else if(state)sNode.checkstate=1;else sNode.checkstate=0;

 z.src=this.imPath+this.checkArray[(state ==3)?2:sNode.checkstate];
};

/**第一个参数为复选框的标签ID
第二个参数为状态 （disable/0:不选, checked/1: 选中, disable/3:禁用 ）
*/
dhtmlXTreeObject.prototype.setSubChecked=function(itemId,state){
 ...
 var sNode=this._globalIdStorageFind(itemId);
 this._setSubChecked(tmp,sNode);
 this._correctCheckStates(sNode.parentObject);
}

 dhtmlXTreeObject.prototype._setSubChecked=function(tmpstate,sNode){
 state=convertStringToBoolean(tmpstate);
 if(!sNode)return;

 for(var i=0;i<sNode.childsCount;i++)
{
 //alert(tmpstate+""+state+""+sNode.childNodes[i].checkstate);
	 if(sNode.childNodes[i].checkstate !=3){
		// alert(sNode.childNodes[i].checkstate +"======"+tmpstate);
	 this._setSubChecked((tmpstate == 3 && sNode.childNodes[i].checkstate ==3)?3:state,sNode.childNodes[i]);}else{
  this._setSubChecked((tmpstate == 3)?3:state,sNode.childNodes[i]);}
};
 var z=sNode.htmlNode.childNodes[0].childNodes[0].childNodes[1].childNodes[0];

 if(tmpstate == 3 ) sNode.checkstate=3;
 else if(state)sNode.checkstate=1;
 else sNode.checkstate=0;
z.src=this.imPath+this.checkArray[(tmpstate == 3)?2:sNode.checkstate];
};
...
function getTree(str,t){
    var roles_a=str.split(",");
    var j=roles_a.length;
    if(str.length<1) return "";
    var newstr=roles_a[0];
    if(t>0){
      for(var i=1;i<=t;i++){
        newstr+=","+roles_a[i];
      }
   ...
   role=newstr;
   for(var i=t;i<j;i++){
	if((i+1)<j && !roles_a[i+1].startsWith(roles_a[t]))
           newstr+=","+roles_a[i+1];
  }

  while(role!=newstr){
    newstr=arguments.callee(newstr,t+1);
  }
return newstr;
}

function getEncodeTreeURI(url, action){
        url = url + "?act=" + action + "&person_id=" + document.forms[0].person_id.value;
        if(document.forms[0].type){
          url += "&type="+document.forms[0].type.value;
        }
        //alert(url);
	if(action.indexOf("update") == -1) return url;

        var roles = getTree(tree.getAllChecked(),0).split(","), j = roles.length;
	for(var i=0; i<j; i++){
          url = url + "&role=" + roles[i];
	}
	return url;
}


function processTreeFinish(request, action){

	try{
		var loop = 0;
		if(action.startsWith("update")){
                  var resptext =  splitTextIntoSpan(request.responseText);
                  if(resptext.length >=1){
                     window.alert(resptext+'\u5df2\u88ab\u5176\u4ed6\u5355\u4f4d\u5b9a\u4e49');
                     var tarr = resptext.split(",");
                     for(var j=0; j<tarr.length; j++){
                      if(tarr[j].length>0)
                        tree.setSubChecked(tarr[j],false);
                      }
                  }else{
                    window.alert('\u64cd\u4f5c\u6210\u529f!');
                  }
			...
			var tmp=getTree(tree.getAllChecked(),0).split(",");

			for(var x=0;x<tmp.length;x++){
				if(tmp[x].length>0)
					tree.setSubChecked(tmp[x],false);
			}
			...
			if(action.startsWith("delete")){
				window.alert('\u64cd\u4f5c\u6210\u529f!');
                          ...
                          var arr   = splitTextIntoSpan(request.responseText).split(",");
                          for(var j=0; j<arr.length; j++){
                            loop ++;
                            if(arr[j].length>0)
                            tree.setSubChecked(arr[j],true);
                          }
			}
		}
	}
