...
GCZLSDCX.define({
	 "setButton": function(iState){
		  ...
		  if((iState==0)||(iState==2)){
			  GCZLSDCX.getTab().find("#add").linkbutton('enable');
		  }else{
			  GCZLSDCX.getTab().find("#add").linkbutton('disable');
		  }
		  
		  //保存
		  if(iState==1){
			  GCZLSDCX.getTab().find("#save").linkbutton('enable');;
		  }else{
			  GCZLSDCX.getTab().find("#save").linkbutton('disable');
		  }
		  
		  //修改
		  if(iState==2){
				GCZLSDCX.getTab().find("#edit").linkbutton('enable');
				GCZLSDCX.getTab().find("#del").linkbutton('enable');
		  }else{
				GCZLSDCX.getTab().find("#edit").linkbutton('disable');
				GCZLSDCX.getTab().find("#del").linkbutton('disable');
		  }
		  
		  //取消
		  if(iState!=0){
			  GCZLSDCX.getTab().find("#cancel").linkbutton('enable');
		  }else{
			  GCZLSDCX.getTab().find("#cancel").linkbutton('disable');
		  }
	},
	"query" : function() {
		var workGrid = GCZLSDCX.getPlugin()["WORKFLOWBECH"];
			workGrid["param"]=$.extend({"userId":userInfo.USERID},GCZLSDCX.getTab().find("form[id='GCZLSDCX']").formToJson()[0]);
		GCZLSDCX.reloadPlugin("WORKFLOWBECH", workGrid);
			
	},
	"add": function(){
		this.setButton(1);
		GCZLSDCX.getTab().find("#billno").textbox("readonly",false);
...
//加载事件
GCZLSDCX.setEvents(function(){
	
});

//加载完成
GCZLSDCX.setAfterInit(function() {  
