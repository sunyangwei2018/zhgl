...
@Controller
@RequestMapping("/BoxItemInterface")
public class BoxItemInterface extends FormTools{
...
//		List<Map> boxnolist = queryForList(tms, querySQL);
	...
	//	cds = new DataSet(excelList.toString());
		String sql=" INSERT INTO bill_boxitem_tmp_tab(billno,boxno,lrr,tbbj)"
				+ " VALUES(billno?, boxno?,'"+CFRY01+"',0) ";
		
		String delall ="delete from bill_boxitem_tmp_tab  where lrr='"+CFRY01+"'";
		...
		String delrepeatbox=" delete from bill_boxitem_tab where boxno=boxno? and billno=billno?";
		execSQL(tms, delrepeatbox, row);	
		sql=" INSERT INTO bill_boxitem_tab(billno,boxno)"
				+ " select a.billno, a.boxno from bill_boxitem_tmp_tab a"
		...
		if(a1>0){
			...
			sql=" update bill_boxitem_tmp_tab set tbbj=1 where lrr='"+CFRY01+"' and tbbj=0 ";
			execSQL(tms, sql, row);
